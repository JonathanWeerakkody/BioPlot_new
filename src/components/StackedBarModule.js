...
function StackedBarModule({ data, options, onOptionsChange }) {
  ...
  useEffect(() => {
    if (data && data.categories && data.datasets) {
      ...
      generateChart(data.categories, data.datasets, { 
        ...options, 
        chartType: options.horizontal ? 'horizontalStackBar' : 'verticalStackBar'
      })
        .then(response => {
  ...
  const downloadChart = () => {
    ...
    if (serverImage) {
      ...
      link.download = `bioplot-${options.horizontal ? 'horizontal' : 'vertical'}-stack-bar-chart.png`;
  ...
  const loadSampleData = () => {
    ...
    onOptionsChange({
      title: `${options.horizontal ? 'Horizontal' : 'Vertical'} Stacked Bar Chart`,
